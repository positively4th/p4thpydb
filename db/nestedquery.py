...
class NestedQuery():
    ...
    @classmethod
    def splitQueries(cls, qs, qSep=None):
    ...
    @classmethod
    def query(cls, executer, qs, qSep=None, *args, **kwargs):
        qOpen, qLoad, qClose = cls.splitQueries(qs, qSep=qSep)
    ...
    def buildCTE(query, queriesMap, uuid=None, escape=r'"'):
        ...
        def createSubQueryReplacer(qName, qSub):
            ...
            def replaceSubQuery(q):
                def onMatch(matches):
                    ...
                    alias = ' _' + \
                        qName if matches[3] is None else ' ' + matches[3]
            ...
            pattern = re.compile(
                NestedQuery.replaceRe0.replace('{{name}}', qName))
        ...
        def createCTEQuery(qName0):
            ...
            if qName0[0] == '=':
                qName = qName0[1:]
                replacements.append(
                    createSubQueryReplacer(qName, queriesMap[qName]))
                return []
            elif qName0[0] in ('+', '-'):
            ...
            q = queriesMap[qName]
            replacement = escape + \
                Util.nextUniq(prefix=qName, suffix=uuid, sep='_') + escape
            replacements.append(createCTEReplacer(
                ':<'+qName0+'>:', replacement))
        ...
        res = whileChanging(
            res, lambda res: applyReplacements(res, replacements))
    ...
    def buildTempQueries(query, queriesMap, uuid=None, escape=r'"', qSep=None):
        ...
        for qName, order in _orderMap.items():
            ...
            orderMap[qName] = max(order, orderMap[qName]
                                  ) if qName in orderMap else order
        parentChildMap = {}
        for qName, children in _parentChildMap.items():
            qName = stripMat(qName)
            cs = [c for c in children]
            parentChildMap[qName] = set(
                cs + parentChildMap[qName]) if qName in parentChildMap else cs
        ...
        def probe(orderMap, parentChildMap, q, qParent, depth):
            ...
            def recurse(match):
                ...
                if not qName in queriesMap:
                    raise NestedQueryMissingQueryError(
                        '{} is not a known query in {}.'.format(qName, q))
                ...
                orderMap[qKey] = max(depth, orderMap[qKey]
                                     if qKey in orderMap else depth)
                orderMap, parentChildMap = probe(
                    orderMap, parentChildMap, queriesMap[qName], qKey, depth+1)

            if depth > maxDepth:
                raise NestedQueryDepthError(
                    '{} seems to be too deep.'.format(query))
