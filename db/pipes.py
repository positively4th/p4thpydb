...
class Pipes():
    ...
    def like(self, qpT, expr, pattern='%', op='LIKE'):
        ...
        pp = dict(p)
        q = 'SELECT * FROM ({}) AS _q WHERE {} {} ({})'.format(q, expr, op, self.util.p(pp, pattern))
        return q, pp, T
    ...
    def any(self, qpT, pipes, cteName=None, op='UNION'):
        ...
        qs = []
        for pipe in pipes:
            qq,p, T = self.util.qpTSplit(self.concat((qPipeed,p), pipes=[pipe]))
    ...
    def aliases(self, qpT, aliasExprMap={}, quote=False):
        ...
        pairs = ['{} AS {}'.format(self.quote(expr, quote), self.quote(alias, quote)) for alias, expr in aliasExprMap.items() if alias and expr] 
